...
public class RedUfoEnemy extends ACreature{
	...
	public float fireIncrement = 10;
	
	private float flightSpeed = 100f;
	
	public RedUfoEnemy(Player player) {
		super(Arrays.asList(Object3DFactory.getUfo()), 15, 3, 2, 0);
		this.player = player;
	...
	public void updateActions(IGameMap map, float timepassed){
		...
		if(aim.x != 0 || aim.y != 0 ){
			if(lastFire > fireIncrement ){
				map.addMapElement(
			    		new FireProjectile(this, getPosition().add(new Vector2f(-.4f,+.4f)), aim)
			    	);
				lastFire = 0;
			}
		}
